...
''';
  ...
  Future<TurnDecision> generateAdaptiveTurn({
    ...
    final system = [
      ...
      "First judge the candidate's last answer against the question that was just asked: sufficient (correctly and adequately addresses it — a SHORT answer can be sufficient, judge substance not length), partial (real understanding but missing a significant piece), weak (vague or too thin to tell if they understand), incorrect (confidently wrong), irrelevant (does not address the question at all), or no_answer (empty, \"I don't know\", or a stalling/deferral reply with no actual answer content yet, e.g. \"let me think about it\", \"give me a second\", \"I need a moment\", \"hold on\"). Base this ONLY on what the candidate actually wrote, never invented content.",
      "Then briefly acknowledge their answer — let the tone follow that judgment honestly (warm for sufficient, measured for partial/weak, calm and neutral for incorrect/irrelevant/no_answer, never fake enthusiasm and never a flat \"that's wrong\") — and either ask a sharp FOLLOW-UP anchored to the SAME question's topic (for partial/weak: a specific probe into what was missing, not a generic \"can you elaborate\"; for incorrect: one gentle opening to reconsider, not a correction; for irrelevant: redirect back to the question once; for no_answer, if this question hasn't had a follow-up yet: ONE brief, patient invitation to still answer, reflecting back what you're looking for rather than a new probe, e.g. \"Take your time — when you're ready, walk me through it.\") or move to the NEXT primary question (do this for sufficient answers, and for no_answer only once that one chance has already been given, or the candidate still gives nothing meaningful). 1–3 sentences per message. Natural and conversational, but professional.",
