...
function scoreColor(s: number) {
  if (s >= 85) return { text: '#0B7A45', bg: '#E2E8F6', bar: 'linear-gradient(90deg,#0B7A45 0%,#BE185D 100%)' }
...
export default function ResultsPage() {
  ...
  function downloadReport() {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>TalbotIQ Report</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Archivo,system-ui,sans-serif;color:#0E1420;background:#EEF0F4;padding:48px}h1{font-size:28px;font-weight:800;letter-spacing:-0.03em;color:#0B7A45;margin-bottom:4px}.meta{font-size:13px;color:#5C6879;margin-bottom:32px}table{width:100%;border-collapse:collapse;font-size:13px;background:#ffffff}td,th{padding:10px 14px;border:1px solid #E3E6ED;text-align:left}th{font-size:11px;text-transform:uppercase;letter-spacing:0.05em;color:#5C6879;background:#F8F9FB}.score{font-size:48px;font-weight:800;letter-spacing:-0.03em;color:#0B7A45;font-variant-numeric:tabular-nums}</style></head><body><h1>TalbotIQ AI Interview Report</h1><p class="meta">Session: ${conv?.conversation_id ?? 'demo'} · Generated: ${new Date().toLocaleString()}</p><p class="score">${overall}<span style="font-size:20px;color:#5C6879">/100</span></p><p style="margin:12px 0 32px;display:inline-block;background:#E2E8F6;color:#152E76;padding:4px 12px;border-radius:9999px;font-size:12px;font-weight:600;border:1px solid #C6D2ED">${verdict}</p><table><tr><th>Dimension</th><th>Score</th><th>Grade</th></tr>${rows}</table></body></html>`
        ...
        <Card className="p-6 flex flex-col items-center">
          <div className="relative w-32 h-32 mb-5">
            <svg width="128" height="128" viewBox="0 0 110 110" style={{ transform: 'rotate(-90deg)' }} aria-hidden="true">
              ...
              <circle cx="55" cy="55" r="48" strokeWidth="7" stroke="#0B7A45" fill="none" strokeLinecap="round"
