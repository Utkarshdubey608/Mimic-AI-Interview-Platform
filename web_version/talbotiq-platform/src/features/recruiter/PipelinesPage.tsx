    ...
    <div className="max-w-[1440px] mx-auto px-6 py-8">
      ...
      <Card className="mb-8 p-4">
        <div className="flex flex-wrap items-end gap-3">
          <span className="hidden h-11 items-center gap-2 pr-1 text-[11px] font-bold uppercase tracking-[0.08em] text-ink-muted sm:inline-flex">
            <ListFilter size={14} className="text-ink" /> Filter
          </span>
          ...
          {!isLoading && !isError && total > 0 && (
            <span className="ml-auto flex h-11 items-center text-xs font-medium tabular-nums text-ink-muted">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p: Pipeline) => (
            <Link
              key={p.id}
              to={`/pipelines/${p.id}`}
              className="group block h-full rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal focus-visible:ring-offset-2"
            >
              <Card hover className="flex h-full flex-col p-5">
                <div className="flex items-start justify-between gap-3">
                  <h2 className="font-display text-[15px] font-bold leading-snug tracking-[-0.01em] text-ink">{p.role}</h2>
                ...
                <ol className="mt-4 flex flex-wrap items-center gap-y-2">
                  {p.rounds.map((r, i) => (
                    <li key={i} className="flex items-center">
                      <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface-sunk py-1 pl-1.5 pr-2.5 text-[11px] font-medium text-ink-body">
                        <span className="flex h-4 w-4 items-center justify-center rounded-full bg-surface-hover text-[9px] font-bold tabular-nums text-ink">{i + 1}</span>
                        <span className="max-w-[9rem] truncate">{r.name}</span>
                      </span>
                      {i < p.rounds.length - 1 && <ChevronRight size={12} aria-hidden className="mx-1 shrink-0 text-ink-disabled" />}
                    </li>
                  ))}
                </ol>
                ...
                <div className="mt-auto flex items-center justify-between gap-3 pt-5">
                  ...
                  <span className="inline-flex items-center gap-1 text-xs font-semibold text-ink-faint transition-colors duration-150 group-hover:text-ink">
